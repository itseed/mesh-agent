...
export async function companionRoutes(fastify: FastifyInstance) {
  ...
  fastify.get('/companion/fs/list', { preHandler }, async (request, reply) => {
    ...
    try {
      ...
      if (
        err.message?.startsWith('Companion RPC timeout') ||
        err.message?.startsWith('Failed to send RPC')
      )
  ...
  fastify.post('/companion/agent/kill', { preHandler }, async (request, reply) => {
    ...
    if (!parseResult.success) return reply.status(400).send({ error: 'sessionId required' });
    try {
      const result = await companionManager.call<{ ok: boolean }>(userId, 'agent.kill', {
        sessionId: parseResult.data.sessionId,
      });
      return result;
    } catch (err: any) {
