...
import { parseReviewIssues, filterNoise } from '@/components/kanban/task-detail/utils';
...
describe('parseReviewIssues', () => {
  ...
  it('drops empty entries from comma-separated lists', () => {
    ...
    expect(parseReviewIssues(body)).toEqual([{ title: 'only-one', severity: 'medium' }]);
  });

  it('parses English summary marker as fallback when Thai missing', () => {
    const body = 'summary: CRITICAL 1 จุด (broken auth)';
    expect(parseReviewIssues(body)).toEqual([{ title: 'broken auth', severity: 'critical' }]);
  });
  ...
  it('does not use the fallback when summary line already produced issues', () => {
    const body = '**สรุป:** HIGH 1 จุด (real issue)\n**1. should not be picked**';
    expect(parseReviewIssues(body)).toEqual([{ title: 'real issue', severity: 'high' }]);
  });
  ...
  it('ignores numbered items whose trimmed title is too short', () => {
    ...
    const body = '**1. a**\n**2. valid title here**';
    expect(parseReviewIssues(body)).toEqual([{ title: 'valid title here', severity: 'medium' }]);
  });

  it('caps fallback title length at 80 chars (longer items skipped)', () => {
    const longTitle = 'x'.repeat(81);
    const body = `**1. ${longTitle}**\n**2. short ok**`;
    expect(parseReviewIssues(body)).toEqual([{ title: 'short ok', severity: 'medium' }]);
  });
});
