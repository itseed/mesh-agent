...
export async function getWaveState(redis: Redis, proposalId: string): Promise<WaveState | null> {
  ...
  if (!raw) return null
  try { return JSON.parse(raw) as WaveState } catch { return null }
}
...
export async function indexSession(redis: Redis, sessionId: string, proposalId: string): Promise<void> {
  await redis.set(sessionIndexKey(sessionId), proposalId, 'EX', WAVE_TTL)
}

export async function lookupSessionProposal(redis: Redis, sessionId: string): Promise<string | null> {
...
function parseResult(stdout: string): WaveEvalResult {
  ...
  return {
    ...
    ask: parsed.ask === true,
    message: typeof parsed.message === 'string' ? parsed.message.trim() : 'Wave evaluation complete.',
  }
}
