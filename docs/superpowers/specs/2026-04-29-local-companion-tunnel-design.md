...
```json
{ "jsonrpc": "2.0", "id": "req-abc123", "method": "fs.list", "params": { "path": "/Users/k/project" } }
```

### Companion → Server (response)

```json
{ "jsonrpc": "2.0", "id": "req-abc123", "result": { "entries": [{ "name": "web", "type": "dir" }, { "name": "api", "type": "dir" }] } }
```

### Error response

```json
{ "jsonrpc": "2.0", "id": "req-abc123", "error": { "code": -32000, "message": "ENOENT: no such file or directory" } }
```
