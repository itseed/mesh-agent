  ...
  connect(): void {
    ...
    this.ws.on('open', () => {
      ...
      this.pingTimer = setInterval(() => {
        this.ws?.send(JSON.stringify({ jsonrpc: '2.0', id: crypto.randomUUID(), method: 'companion.ping', params: {} }))
      }, PING_INTERVAL_MS)
    })
    ...
    this.ws.on('message', async (raw: Buffer) => {
      ...
      if (!handler) {
        this.send({ jsonrpc: '2.0', id: req.id, error: { code: -32601, message: `Method not found: ${req.method}` } })
        return
      }
      ...
      try {
        ...
        this.send({ jsonrpc: '2.0', id: req.id, error: { code: -32000, message: err.message ?? 'Internal error' } })
      }
    })
